...
export default function MergedCommand({ base = 'main', dryRun, verbose, yes }: CommandFlags) {
  ...
  useEffect(() => {
    async function fetchMergedWorktrees() {
      try {
        const manager = new WorktreeManager();
        const merged = await manager.findMergedBranchWorktrees(base);
        setMergedWorktrees(merged);

        const hasNoTargets = merged.length === 0;
  ...
  useEffect(() => {
    ...
    if (isCleanupStage) {
      async function performCleanup() {
        const manager = new WorktreeManager();
        const cleanupResult = await manager.removeWorktrees(mergedWorktrees, { dryRun });
  ...
  const hasNoTargets = mergedWorktrees.length === 0;
  if (hasNoTargets) {
    return <Text color="green">✓ No merged branch worktrees found (base: {base})</Text>;
  }
  ...
  if (isDisplayStage) {
    ...
    return (
      <Box flexDirection="column">
        <Text color="yellow">
          Found {mergedWorktrees.length} worktrees with branches merged to {base}:
        </Text>
        <WorktreeTable worktrees={mergedWorktrees} verbose={verbose} />
        <Text>
          {isDryRunMode ? (
  ...
  if (isDryRunMode) {
    return (
      <Box flexDirection="column">
        <Text color="blue">[DRY RUN] Would remove {mergedWorktrees.length} worktrees</Text>
